...
function exportarExcel() {
  ...
  // comprobó que la librería de Excel disponible en el navegador no
  // conserva ese formato al escribir el archivo (lo ignora silenciosamente).
  // Por eso se usa un indicador visual con emoji (🟦 Teoría / 🟥 Práctica)
  // que sí se ve igual en cualquier programa, sin depender de esa función.
  const INDICADOR_TEORIA = '🟦';
  const INDICADOR_PRACTICA = '🟥';
    ...
    DIAS_XLS.forEach(dia => {
      ...
      if (bloques.length) {
        fila.push(bloques.map(b => {
          ...
          const indicador = esTeoria ? INDICADOR_TEORIA : INDICADOR_PRACTICA;
          const tipoLetra = esTeoria ? 'T' : 'P';
          return `${b.sec.nombre}\nSección ${b.sec.seccion} · ${indicador} ${tipoLetra}`;
        }).join('\n\n'));
      } else {
        fila.push('');
      }
  ...
  wsCalendario['!cols'] = [{ wch: 9 }, ...DIAS_XLS.map(() => ({ wch: 24 }))];
  wsCalendario['!rows'] = [{ hpx: 24 }, ...horas.map(() => ({ hpx: 46 }))];
  ...
  wsDetalle['!cols'] = [
    { wch: 10 }, // Código
    { wch: 28 }, // Curso (con ajuste de línea)
    { wch: 9 },  // Sección
    { wch: 26 }, // Docente
    { wch: 10 }, // Tipo
    { wch: 12 }, // Día
  ...
  wsDetalle['!rows'] = [{ hpx: 22 }, ...detalleData.slice(1).map(() => ({ hpx: 32 }))];
